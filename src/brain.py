...
class Brain:
    ...
    async def process_command(self, user_input: str):
        ...
        if "trouble thinking locally" not in local_result.get("speech", "") and "formatting issue" not in local_result.get("speech", ""):
            return local_result
    ...
    async def _process_local(self, user_input: str):
        ...
        # Prepend system prompt to the first message in history or current input
        if not self.history:
            combined_content = f"SYSTEM INSTRUCTIONS:\n{self.system_prompt}\n\nUSER INPUT:\n{user_input}"
            messages.append({"role": "user", "content": combined_content})
        else:
            # We skip the system role entirely to avoid 400 errors from strict servers
            # Just add history normally
        ...
        payload = {
            ...
            "messages": messages,
            "temperature": 0.7,
            "max_tokens": 512, # Reduced to save context space
            "stream": False
        }
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return {
                    "speech": "I'm having a context limit issue locally.",
                    "command": None,
                    "thought": f"Server Error {response.status_code}: {response.text}"
                }
                
            data = response.json()
            content = data['choices'][0]['message']['content']

            # Record in history (Stricter limit for local context)
            self.history.append({"role": "user", "content": user_input})
            self.history.append({"role": "assistant", "content": content})
            if len(self.history) > 8: # Keep only last 4 turns (8 messages)
    ...
    async def _process_gemini(self, user_input: str):
        """Handles inference via Google Gemini API with aggressive pacing and retries."""
        ...
        for attempt in range(max_retries + 1):
            try:
                # 10 second delay between turns to stay under free tier RPM
                print(f"[*] Pacing Gemini (10s delay)... attempt {attempt+1}")
                ...
                if "429" in error_msg:
                    if attempt < max_retries:
                        print("[*] Quota hit. Sleeping 30s before retry...")
