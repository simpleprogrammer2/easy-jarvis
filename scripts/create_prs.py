...
class GitHubAPI:
    ...
    def _run_curl(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
...
def generate_pr_title(branch_name: str) -> str:
    ...
    clean_name = branch_name.rsplit("-", 1)[0] if branch_name[-10:].isdigit() else branch_name
...
def main():
    ...
    parser.add_argument(
        "--repo",
        default="easy-jarvis",
        help="Repository name (default: easy-jarvis)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview PRs without creating them"
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
    ...
    for i, branch in enumerate(branches, 1):
        ...
        if args.skip_existing and api.get_existing_prs(branch):
            print(f"  ⏭️  Skipping (PR already exists)")
            skipped += 1
            continue
